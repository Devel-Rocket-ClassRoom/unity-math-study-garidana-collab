...
{
    [Header("=== 벡터 입력 ===")] // 벡터 입력 헤더 (유니티 인스펙터에서 툴팁 표시)
    [Tooltip("첫 번째 벡터 (파란색 화살표)")]
    [SerializeField] // 벡터 입력 필드 (유니티 인스펙터에서 표시)
     private Vector3 vectorA = new Vector3(3f, 0f, 1f);
    ...
    private void OnValidate() // 유니티 이벤트 함수 (유니티 인스펙터에서 값이 변경될 때 호출)
    {
        PrintVectorOperations(); // 벡터 연산 결과 출력
    }

    private void PrintVectorOperations() // 벡터 연산 결과 출력 함수
    {
        // TODO
        sum = vectorA + vectorB;  // 덧셈 연산
        diff = vectorA - vectorB;  // 뺄셈 연산
        scaled = scalar * vectorA; // 스칼라 곱 연산
