...
public class Assignment_UIPopup : MonoBehaviour
{
    ...
    // RectTransform은 Transform을 상속받기에 Scale이 Vector3 타입임.
    ...
    private void UpdateAnimation()
    {
        ...
        if (currentState == PopupState.Showing) // 페이드 인 
        {
            // TODO
            float easedOutT = EaseOut(t);
            popupPanel.localScale = Vector3.one * easedOutT;
            canvasGroup.alpha = easedOutT;
        ...
        else if (currentState == PopupState.Hiding) // 페이드 아웃
        {
            // TODO
            float easedInT = EaseIn(t);
            popupPanel.localScale = Vector3.one * (1f - easedInT);
            canvasGroup.alpha = 1f - easedInT;
